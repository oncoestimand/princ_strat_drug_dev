...
prettyCox <- function(mod, dig.coef = 2, dig.p = 1){
  ...
  colnames(tab1) <- c("Variable", "Coefficient", "HR = exp(coef)", "95\\% CI", 
                      "$p$-value")
...
legend("topright", legend = c("Z = 1, S(1) = 1", 
                              "Z = 0 (weighted by P(S(1) = 1))", "Z = 0 (all)"),
       col = 1:3, lwd = 2)
...
approach <- c("Truth", "Naive", "Regression adjustment", "Weighting", 
              "Multiple imputation")
est <- c(coef(truth_S1)[1], coef(naive)[1], coef(reg_adj)[1], coef(weighting), 
         mi[1])
se <- c(NA, sqrt(vcov(naive)), sqrt(vcov(reg_adj)[1, 1]), 
        sqrt(vcov(weighting)[1, 1]), mi[3])
tab <- data.table(approach = approach, estimate = round(est, 3), se = round(se, 3), 
                  "hazard ratio" = round(exp(est), 3))
kable(tab)
```
...
hr <- numeric(nSim)
for(i in 1:nSim){
  pp <- rbeta(1, sm + 1 / 3, n1 - sm + 1 / 3) ## sample from posterior for P(S1 = 1) 
